"""
RMOL scraper — uses tag-based HTML parsing for search.

https://rmol.id/tag/{keyword}
Article pattern: /{category}/read/{year}/{month}/{day}/{id}/{slug}
"""
...
class RMOLScraper(BaseScraper):
    def __init__(self, keywords, concurrency=5, start_date=None, queue_=None):
        super().__init__(keywords, concurrency, queue_)
        self.base_url = "https://rmol.id"
        self.start_date = start_date
        self.continue_scraping = True
        self.max_pages = 10
        self._article_re = re.compile(
            r"^https?://rmol\.id/[a-z]+/read/\d+/\d+/\d+/\d+/.+"
        )

    async def build_search_url(self, keyword, page):
        if page == 1:
            url = f"{self.base_url}/tag/{keyword}"
        else:
            url = f"{self.base_url}/tag/{keyword}/page/{page}"
    ...
    async def get_article(self, link, keyword):
        ...
        for tag in content_div.find_all(
            ["section", "div", "footer"],
            class_=re.compile(
                r"related|popular|most|sidebar|share|social|newsletter|ad|subscribe|embed|block_related|author-box|post-tags|comments|wp-block|tags-list|related-article",
                re.IGNORECASE,
            ),
        ):
            tag.extract()
    ...
    def _extract_date(self, soup, link):
        ...
        logging.debug("RMOL date parse failed | url: %s", link)
        return None
