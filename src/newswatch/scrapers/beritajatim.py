"""
BeritaJatim scraper — uses tag page with keyword filtering.

https://beritajatim.com/tag/{keyword}
"""
...
class BeritaJatimScraper(BaseScraper):
    def __init__(self, keywords, concurrency=5, start_date=None, queue_=None):
        super().__init__(keywords, concurrency, queue_)
        self.base_url = "https://beritajatim.com"
        self.start_date = start_date
        self.continue_scraping = True
        self.max_pages = 10
        self._article_re = re.compile(r"https?://beritajatim\.com/(?!kanal/|tag/|author/|category/|page/|arsip)[a-z0-9-]+/?$")
    ...
    async def get_article(self, link, keyword):
        ...
        if not publish_date:
            logging.debug("BeritaJatim date parse failed | url: %s | date: %r", link, publish_date_str[:50])
            return
