"""
Niaga.Asia scraper — uses WordPress search with HTML parsing.

https://www.niaga.asia/?s={keyword}
...
class NiagaAsiaScraper(BaseScraper):
    def __init__(self, keywords, concurrency=5, start_date=None, queue_=None):
        super().__init__(keywords, concurrency, queue_)
        self.base_url = "https://www.niaga.asia"
        self.start_date = start_date
        self.continue_scraping = True
        self.max_pages = 10
        self._skip_paths = [
            "/wp-", "/search/", "/tag/", "/author/", "/feed/",
            "/xmlrpc", "/category/", "/topik/", "/page/", "/redaksi/",
            "/about-us/", "/info-produk/", "/disclaimer/", "/pedoman-",
            "/artikel-opini/", "/comments/feed/", "/cdn-cgi/",
        ]
        self._article_re = re.compile(
            r"^https?://www\.niaga\.asia/[a-z0-9-]+/?$"
        )
    ...
    async def get_article(self, link, keyword):
        ...
        for tag in content_div.find_all(
            ["section", "div", "footer"],
            class_=re.compile(
                r"related|popular|most|sidebar|share|social|newsletter|ad|subscribe|embed|block_related|author-box|post-tags|comments|wp-block",
                re.IGNORECASE,
            ),
        ):
            tag.extract()
    ...
    def _extract_date(self, soup, link):
        ...
        logging.debug("NiagaAsia date parse failed | url: %s", link)
        return None
