"""
Merdeka.com scraper — uses RSS feed with keyword filtering.

The CSE approach is rate-limited, but RSS items can be filtered
...
class MerdekaScraper(BaseScraper):
    ...
    async def fetch_search_results(self, keyword):
        """Fetch RSS feed and filter by keyword."""
        kw_lower = keyword.lower()
        all_items = []

        try:
            response_text = await self.fetch(self.rss_url, headers=self.headers, timeout=30)
            ...
            logging.debug(f"Merdeka RSS fetch failed: {e}")
