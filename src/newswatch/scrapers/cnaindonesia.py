...
class CNAIndonesiaScraper(BaseScraper):
    def __init__(self, keywords, concurrency=5, start_date=None, queue_=None):
        super().__init__(keywords, concurrency, queue_)
        self.base_url = "https://www.cna.id"
        self.start_date = start_date
        self.continue_scraping = True
        self.max_pages = 10
        self._article_re = re.compile(
            r"^https?://www\.cna\.id/[a-z-]+/[a-z0-9-]+-[0-9]+$"
        )
    ...
    async def get_article(self, link, keyword):
        ...
        for tag in content_div.find_all(
            ["section", "div", "footer"],
            class_=re.compile(
                r"related|popular|most|sidebar|share|social|newsletter|ad|subscribe|embed|block_related|author-box|post-tags|comments|wp-block|related-article|tags-list",
                re.IGNORECASE,
            ),
        ):
            tag.extract()
    ...
    async def build_latest_url(self, page):
        if page == 1:
            return await self.fetch(f"{self.base_url}/terbaru")
        return None
