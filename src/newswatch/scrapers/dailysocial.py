"""
DailySocial scraper — uses WordPress search with HTML parsing.

https://news.dailysocial.id/?s={keyword}
Article pattern: /post/{slug}
"""
...
class DailySocialScraper(BaseScraper):
    def __init__(self, keywords, concurrency=5, start_date=None, queue_=None):
        super().__init__(keywords, concurrency, queue_)
        self.base_url = "https://news.dailysocial.id"
        self.start_date = start_date
        self.continue_scraping = True
        self.max_pages = 10
        self._article_re = re.compile(
            r"^https?://news\.dailysocial\.id/post/.+$"
        )

    async def build_search_url(self, keyword, page):
        params = {"s": keyword}
        if page > 1:
            url = f"{self.base_url}/page/{page}/?{urlencode(params)}"
        else:
        ...
        for item in items:
            ...
            if a:
                ...
                if self._article_re.match(full_url):
                    ...
                    if not any(x in full_url for x in ["/tag/", "/category/", "/author/"]):
                        links.add(full_url)
    ...
    async def get_article(self, link, keyword):
        ...
        content_div = soup.select_one(".entry-content") or soup.select_one(".wp-block-post-content")
        if not content_div:
            return
        ...
        for tag in content_div.find_all(
            ["section", "div"],
            class_=re.compile(
                r"related|popular|most|sidebar|share|social|newsletter|ad|subscribe|embed|author|jetpack",
                re.IGNORECASE,
            ),
        ):
            tag.extract()
    ...
    def _extract_date(self, soup, link):
        ...
        logging.debug("DailySocial date parse failed | url: %s", link)
        return None
    ...
    def _extract_author(self, soup):
        ...
        # Try post author name block
        author_el = soup.select_one(".wp-block-post-author-name a")
        if author_el:
    ...
    def parse_latest_article_links(self, response_text):
        ...
        for item in items:
            ...
            if a:
                href = a["href"]
                full_url = urljoin(self.base_url, href) if not href.startswith("http") else href
                if self._article_re.match(full_url) and "/tag/" not in full_url and "/category/" not in full_url:
                    links.add(full_url)
        return links or None
